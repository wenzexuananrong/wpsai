...
<div class="acfwp-store-credit-reminder-template">
    ...
    <?php if ( count( $promotion ) ) : ?>
        <div class="acfwp-promotion-type-<?php echo esc_attr( $promotion_type ); ?>" style="border-top: 1px solid #e6e6e6; padding-top: 1.5rem; text-align:center;">
            <?php echo wp_kses_post( $promotion_text ); ?>
            <table style="width:100%; padding:1rem;">
                <tr>
                    <?php foreach ( $promotion as $product ) : ?>
                        <td style="width:33.33%; padding:1rem; text-align: center; vertical-align:top;">
                            <?php
                                $featured_image = wp_get_attachment_image_src( $product->get_image_id(), 'full' );
                                if ( $featured_image && isset( $featured_image[0] ) ) :
                            ?>
                                <img src="<?php echo esc_url( $featured_image[0] ); ?>" alt="<?php echo esc_attr( $product->get_title() ); ?>" style="border-radius: .25rem;">
                            <?php endif; ?>
                            <h3 style="text-align:center; color: #4d504d; padding: .5rem;"><?php echo esc_html( $product->get_title() ); ?></h3>
                            <p style="color: #4d504d; font-size: .75rem;"><?php echo wp_kses_post( $product->get_price_html() ); ?></p>
                            <p style="text-align:center;">
                                <a href="<?php echo esc_url( $product->get_permalink() ); ?>" style="cursor: pointer;display: inline-block;padding: .75rem 1.25rem;text-decoration: none;font-weight: 600;background-color: #ebebeb;border-color: #ebebeb;color: #4d504d;font-size: .75rem;">
