...
class Update implements Model_Interface, Initiable_Interface, Deactivatable_Interface {
    ...
    public function ajax_refetch_update_data() {
        if ( ! defined( 'DOING_AJAX' ) || ! DOING_AJAX ) {
            $response = array(
                'status'    => 'fail',
                'error_msg' => __( 'Invalid AJAX call', 'loyalty-program-for-woocommerce' ),
            );
        } elseif ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_key( $_POST['nonce'] ), 'lpfw_slmw_refetch_update_data' ) || ! current_user_can( 'manage_woocommerce' ) ) {
            $response = array(
                'status'    => 'fail',
