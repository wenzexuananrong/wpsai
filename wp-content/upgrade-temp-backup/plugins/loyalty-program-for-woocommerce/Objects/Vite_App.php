...
class Vite_App {
    ...
    public function __construct(
         ...
         */
        $this->is_hmr = defined( 'HMR_DEV' ) && 'lpfw' === HMR_DEV;
        ...
        $this->base_url = $this->is_hmr
            ? "$protocol//$host:$port/"
            : plugins_url( 'dist/', LPFW()->Plugin_Constants->MAIN_PLUGIN_FILE_PATH );
    ...
    protected function parse_env() {
         ...
         */
        if ( ! file_exists( LPFW()->Plugin_Constants->PLUGIN_DIR_PATH . '.env' ) ) {
            return;
        }

        //phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents
        $env = file_get_contents( LPFW()->Plugin_Constants->PLUGIN_DIR_PATH . '.env' );
        if ( $env ) {
            $env = preg_split( '/\\r\\n|\\r|\\n/', $env );
    ...
    protected function parse_manifest_file() {
         ...
         */
        $manifest_path = LPFW()->Plugin_Constants->PLUGIN_DIR_PATH . 'dist/manifest.json';
            ...
            foreach ( $imports as $import ) {
                ...
                if ( ! empty( $import_styles ) ) {
                    foreach ( $import_styles as $import_style ) {
                        ...
                        wp_enqueue_style( "$this->script_handle-import-$sanitized_key", $css_url, array(), filemtime( LPFW()->Plugin_Constants->MAIN_PLUGIN_FILE_PATH ) );
                    }
                }
            }
        }
    ...
    protected function enqueue_hmr_vite_client() {
        ...
        wp_enqueue_script(
            ...
            array(),
            filemtime( LPFW()->Plugin_Constants->MAIN_PLUGIN_FILE_PATH ),
            false
        );
    ...
    protected function enqueue_main_script() {
        ...
        $script_version = $this->is_hmr
            ? time()
            : filemtime(
                LPFW()->Plugin_Constants->PLUGIN_DIR_PATH .
        ...
        wp_localize_script(
            ...
            array_merge(
                array(
                    'pluginDirUrl' => plugins_url( '', LPFW()->Plugin_Constants->MAIN_PLUGIN_FILE_PATH ),
        ...
        if ( in_array( 'wp-i18n', $this->dependencies, true ) ) {
            wp_set_script_translations(
                ...
                LPFW()->Plugin_Constants->PLUGIN_DIR_PATH . 'languages'
            );
        }
        ...
        if ( ! $this->is_hmr ) {
            ...
            if ( ! empty( $styles ) ) {
                foreach ( $styles as $style ) {
                    wp_enqueue_style(
                        $this->script_handle,
                        plugins_url( "dist/$style", LPFW()->Plugin_Constants->MAIN_PLUGIN_FILE_PATH ),
                        $this->style_dependencies,
                        filemtime( LPFW()->Plugin_Constants->PLUGIN_DIR_PATH . "dist/$style" )
                    );
                }
            }
            ...
            if ( $this->is_hmr ) {
                ...
                if ( apply_filters( 'acfw_enable_subresource_integrity_check', false ) ) {
                    $integrity = sprintf(
                        ...
                        $this->get_file_hash(
                            LPFW()->Plugin_Constants->PLUGIN_DIR_PATH . "dist/{$this->manifest[ $this->entry_file_path ]['file']}"
                        )
                    );
                }
                $tag = str_replace(
                ...
                if ( false !== strpos( $href, $style ) ) {
                    ...
                    if ( apply_filters( 'acfw_enable_subresource_integrity_check', false ) ) {
                        $integrity = sprintf(
                            'crossorigin="anonymous" integrity="%s" ',
                            $this->get_file_hash(
                                LPFW()->Plugin_Constants->PLUGIN_DIR_PATH . "dist/$style"
                            )
                        );
                    }
