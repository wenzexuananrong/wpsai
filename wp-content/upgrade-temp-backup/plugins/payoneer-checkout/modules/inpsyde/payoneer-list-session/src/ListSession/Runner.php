...
class Runner implements ListSessionProvider, ListSessionPersistor
{
    ...
    public function persist(?ListInterface $list, ContextInterface $context): bool
    {
         ...
         */
        if (
            $middleware instanceof ListSessionMiddleware
            && !$middleware instanceof ListSessionPersistorMiddleware
        ) {
        ...
        throw new RuntimeException(
            sprintf(
                ...
                ListSessionPersistorMiddleware::class
            )
        );
    }
    ...
    public function provide(ContextInterface $context): ListInterface
    {
        ...
        if ($middleware === false) {
            throw new RuntimeException(
                'Failed to provide List session, no suitable provider found.'
            );
        }
        next($this->middlewares);
        /**
        ...
        if (
            $middleware instanceof ListSessionMiddleware
            && !$middleware instanceof ListSessionProviderMiddleware
        ) {
        ...
        throw new RuntimeException(
            sprintf(
                'Invalid middleware queue entry: %s. Middleware must either be callable or implement %s.',
                get_class($middleware),
                ListSessionProviderMiddleware::class
            )
        );
    }
}
