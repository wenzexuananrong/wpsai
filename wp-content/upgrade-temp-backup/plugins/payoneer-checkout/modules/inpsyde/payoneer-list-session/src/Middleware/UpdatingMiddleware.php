...
use Inpsyde\PayoneerForWoocommerce\ListSession\Factory\ListSession\WcBasedUpdateCommandFactoryInterface;
...
class UpdatingMiddleware implements ListSessionProviderMiddleware
{
    ...
    public function provide(
        ...
        try {
            $updated = $this->updateExistingListSession(
                $list,
                $context->getCart(),
                $context->getCustomer()
            );
    ...
    protected function updateExistingListSession(
        ...
        do_action('payoneer-checkout.list_session_updated', [
            'longId' => $updatedList->getIdentification()->getLongId(),
            'list' => $updatedList,
        ]);
