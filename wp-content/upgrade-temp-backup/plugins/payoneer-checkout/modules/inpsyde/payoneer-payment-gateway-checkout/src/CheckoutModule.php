...
class CheckoutModule implements ServiceModule, ExecutableModule, ExtendingModule
{
    ...
    protected function registerAddingGatewayTitleExtraMarkup(ContainerInterface $container): void
    {
        ...
        add_filter(
            ...
            static function (string $title, string $id) use ($container) {
                ...
                $isGetRequest = isset($_SERVER['REQUEST_METHOD'])
     ...
     * @return void
     */
    protected function registerClearingListIfHppFallbackFlagSet(
    ...
    protected function registerClearingListOnFailedOrderPayment(
        ContainerInterface $container
    ): void {
    ...
    protected function registerMovingListToCreatedOrder(ContainerInterface $container): void
    {
        add_action('woocommerce_checkout_order_processed', static function (
            $_orderId, //underscore is for psalm to not complain about unused variable
            $_postedData,
            WC_Order $order
        ) use ($container) {
    ...
    protected function setupCheckoutActions(ContainerInterface $container): void
    {
        ...
        add_action(
            ...
            function (int $orderId) use ($container) {
                ...
                $listSessionManager = $container->get('list_session.manager');
                assert($listSessionManager instanceof ListSessionManager);
                $chargeIdFieldName = (string)$container->get(
