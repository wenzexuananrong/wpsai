...
class AnalyticsModule implements ServiceModule, ExtendingModule, ExecutableModule
{
    ...
    public function run(ContainerInterface $container): bool
    {
        ...
        add_action('woocommerce_init', function () use ($container): void {
            ...
            $trackedHooksConfig = apply_filters(
                'payoneer-checkout.analytics_events',
                $container->get('analytics.analytics_events')
            );
    ...
    public function registerOrderCreatedFromCheckoutListener(
        ...
        add_filter(
            ...
            function (
                ...
                $gaSessionFieldName = (string) $container->get(
                    'analytics.ga_session_id_order_field_name'
                );
                ...
                do_action('payoneer-checkout.order_created_from_checkout', [
                    'BILLING_COUNTRY' => $order->get_billing_country(),
                    'CURRENCY' => $order->get_currency(),
                    'PAYMENT_METHOD_ID' => $paymentMethod,
                    'TOTAL_AMOUNT' => (float) $order->get_total(''),
                    'GA_SESSION_ID' => $gaSessionId,
                    'STORE_CODE' => (string) $container->get('analytics.merchant_division'),
                ]);
    ...
    public function registerOrderFailedFromCheckoutListener(ContainerInterface $container): void
    {
        add_action('shutdown', function () use ($container): void {
            ...
            do_action(
                ...
                [
                    'BILLING_COUNTRY' => filter_input(
                        INPUT_POST,
                        'billing_country',
                        FILTER_CALLBACK,
                        ['options' => 'strip_tags']
                    ),
                    'PAYMENT_METHOD_ID' => $this->getChosenPaymentMethod($container),
                    'CURRENCY' => $container->get('analytics.store_currency'),
                    'TOTAL_AMOUNT' => $this->getCartTotal($container),
                    'STORE_CODE' => (string) $container->get('analytics.merchant_division'),
                ],
                PHP_INT_MAX
            );
    ...
    public function registerOrderPayFormSubmittedListener(
        ContainerInterface $container
    ): void {
        ...
        do_action('payoneer-checkout.pay_for_order_form_submitted', [
            'BILLING_COUNTRY' => $order->get_billing_country(),
            'CURRENCY' => $order->get_currency(),
            'TOTAL_AMOUNT' => (float) $order->get_total(''),
            'PAYMENT_METHOD_ID' => $paymentMethod,
    ...
    public function registerOrderStatusChangedToProcessingListener(ContainerInterface $container): void
    {
        add_action(
            ...
            static function (
                ...
                do_action('payoneer-checkout.order_status_changed_to_processing', [
                        'BILLING_COUNTRY' => $order->get_billing_country(),
                        'CURRENCY' => $order->get_currency(),
                        'TOTAL_AMOUNT' => (float) $order->get_total(''),
    ...
    public function registerOrderStatusChangedToFailedListener(ContainerInterface $container): void
    {
        ...
        add_action(
            ...
            static function (
                ...
                do_action('payoneer-checkout.order_status_changed_to_failed', [
                    'BILLING_COUNTRY' => $order->get_billing_country(),
                    'PAYMENT_METHOD_ID' => $order->get_payment_method(),
                    'CURRENCY' => $order->get_currency(),
                    'TOTAL_AMOUNT' => (float) $order->get_total(''),
    ...
    public function registerPluginActivatedListener(ContainerInterface $container): void
    {
        add_action('wp', static function () use ($container): void {
            if (get_option('payoneer-checkout_plugin_activated')) {
                ...
                do_action('payoneer-checkout_plugin_activated', [
                    'PLUGIN_VERSION' =>
                        (string) $container->get('analytics.plugin_version_string'),
                ]);
            }
        });
    }
    ...
    public function registerCheckoutViewedListener(ContainerInterface $container): void
    {
        add_action('shutdown', function () use ($container): void {
            ...
            $total = $container->get('analytics.is_checkout_pay_page') ?
                $this->getTotalOnOrderPayPage($container) :
                $this->getCartTotal($container);

            do_action('payoneer-checkout.checkout_page_viewed', [
                'CURRENCY' => $container->get('analytics.store_currency'),
                'TOTAL_AMOUNT' => $total,
                'PAGE_LOCATION' => (string) $container->get('analytics.http.current_url'),
                'PAYMENT_METHOD_ID' => $this->getChosenPaymentMethod($container),
                'STORE_CODE' => (string) $container->get('analytics.merchant_division'),
            ]);
        });
    ...
    protected function getTotalOnOrderPayPage(ContainerInterface $container): float
    {
        ...
        $orderUnderPayment = wc_get_order(
            $container->get('analytics.order_under_payment.id')
        );
    ...
    protected function ensureOrderHasGaSessionId(
        WC_Order $order,
        string $gaSessionFieldName,
     ...
     */
    public function setUpTrackedHooksHandling(
        AnalyticsEventHandlerInterface $eventHandler,
        array $trackedHooksConfig
    ): void {
