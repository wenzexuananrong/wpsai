...
namespace Inpsyde\PayoneerForWoocommerce\Analytics\AnalyticsApiClient;
...
class AnalyticsApiClient implements AnalyticsApiClientInterface
{
    ...
    public function post(string $payload): void
    {
        try {
            ...
            do_action(
                'payoneer_checkout.analytics_request_failed',
                ['exception' => $exception]
            );
        ...
        do_action(
            'payoneer_checkout.analytics_request_sent',
            [
                'request' => $request,
                'response' => $response,
            ]
    ...
    protected function createBodyFromPayload(string $payload): StreamInterface
    {
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            throw new InvalidArgumentException(sprintf(
                'Provided string "%1$s" is not a valid JSON.',
                $payload
            ));
        }
