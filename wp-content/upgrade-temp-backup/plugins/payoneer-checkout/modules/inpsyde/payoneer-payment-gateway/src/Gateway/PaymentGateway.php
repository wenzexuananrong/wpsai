...
class PaymentGateway extends WC_Payment_Gateway
{
    ...
    {
        $title = $this->options->get('live_mode') === 'no' ?
            __('Test:', 'payoneer-checkout') . ' ' . $this->title :
            $this->title;
    ...
    public function process_refund($orderId, $amount = \null, $reason = '')
    {
        ...
        try {
            $refundProcessor = $this->serviceContainer
                ->get('inpsyde_payment_gateway.refund_processor');
    ...
    {
        $apiCredentialsValidator = $this->serviceContainer
            ->get('inpsyde_payment_gateway.api_credentials_validator');
        assert($apiCredentialsValidator instanceof ValidatorInterface);

        $apiCredentialsValidator->validate([
            'code' => $code,
            'token' => $token,
            'url' => $url,
            'division' => $division,
