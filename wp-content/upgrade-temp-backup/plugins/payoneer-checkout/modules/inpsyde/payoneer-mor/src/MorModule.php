...
namespace Inpsyde\PayoneerForWoocommerce\Mor;
...
class MorModule implements ExecutableModule, ServiceModule, ExtendingModule
{
     ...
     */
    protected function addTaxCodeFieldToProduct(
        string $taxCodeFieldName,
        string $taxCodeFormat,
        string $taxCodeFieldTitle
    ): void {

        add_action(
            'woocommerce_product_options_general_product_data',
            static function () use ($taxCodeFieldName, $taxCodeFormat, $taxCodeFieldTitle): void {
                echo '<div class="payoneer-checkout.tax-code options_group">';
                woocommerce_wp_text_input(
                    [
                        'id' => $taxCodeFieldName,
                        'placeholder' => __(
                            'Tax code to apply on this product',
                            'payoneer-checkout'
                        ),
                        'label' => __('Tax code', 'payoneer-checkout'),
                        'desc_tip' => 'true',
                        'custom_attributes' => [
                            'pattern' => $taxCodeFormat,
                            'title' => $taxCodeFieldTitle,
                        ],
                    ]
                );
     ...
     * @param string $taxCodeFieldTitle
     */
    protected function addTaxCodeFieldToVariation(
        string $taxCodeFieldName,
        string $taxCodeFormat,
        string $taxCodeFieldTitle
    ): void {
        ...
        add_action(
            ...
            static function ($loop, $variationData, $variation) use ($taxCodeFieldName, $taxCodeFormat, $taxCodeFieldTitle): void {
                ...
                woocommerce_wp_text_input([
                    'id' => 'variation' . $taxCodeFieldName . '[' . $loop . ']',
                    'class' => 'short',
                    'placeholder' => __('Tax code to apply on this product', 'payoneer-checkout'),
                    'label' => __('Tax Code', 'payoneer-checkout'),
                    'value' => get_post_meta(
                        (int) $variation->ID,
                        $taxCodeFieldName,
    ...
    protected function registerProductTaxCodeSaving(string $taxCodeFieldName): void
    {
        $callback = static function ($postId) use ($taxCodeFieldName): void {
            ...
            $taxcode = (isset($_POST[$taxCodeFieldName]) && is_string($_POST[$taxCodeFieldName])) ?
                sanitize_text_field(wp_unslash($_POST[$taxCodeFieldName])) :
                null;
        ...
        foreach (['simple', 'variable'] as $productType) {
            add_action(
                "woocommerce_process_product_meta_$productType",
                $callback
            );
        }
    }
    ...
    protected function registerProductVariationTaxCodeSaving(string $taxCodeFieldName): void
    {
        ...
        add_action(
            ...
            static function ($variationId, $i) use ($taxCodeFieldName): void {
                $postedFieldName = 'variation' . $taxCodeFieldName;
                //@phpcs:disable WordPress.Security.NonceVerification.Missing
                /**
                 * @psalm-suppress MixedArrayOffset
                 */
                $taxCode = isset($_POST[$postedFieldName][$i])
                               && is_string($_POST[$postedFieldName][$i])
                    ? sanitize_text_field(wp_unslash($_POST[$postedFieldName][$i])) : null;
