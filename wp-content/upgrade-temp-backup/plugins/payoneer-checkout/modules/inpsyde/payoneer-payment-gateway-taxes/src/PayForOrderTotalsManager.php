...
class PayForOrderTotalsManager
{
    public function __invoke(
        ...
        foreach ($totals as $totalLineId => $total) {
            ...
            if (
                str_contains((string)$totalLineId, 'tax')
            ) {
                if (!$payoneerTaxAlreadyAdded && $listTaxAmount > 0) {
                    $payoneerTotals = $this->setTaxLine(
                        $payoneerTotals,
                        $payoneerTotalKey,
                        $listTaxAmount
                    );
                    $payoneerTaxAlreadyAdded = true;
                }
                ...
                $payoneerTotals[$payoneerTotalKey] = [
                    'label' => $this->payoneerTotalLine($total['label']),
                    'value' => wc_price($listAmount),
                ];
            } else {
                $payoneerTotals[$payoneerTotalKey] = [
                    'label' => $this->payoneerTotalLine($total['label']),
    ...
    protected function payoneerTotalLine(string $value): string
    {
        ...
        return '<span class="payoneer-total">' .
            $value .
            '</span>';
    }
    ...
    {
        $totals[$taxLineKey] = [
            'label' => '<span class="payoneer-total">' .
                __('Tax:', 'payoneer-checkout') .
                '</span>',
            'value' => wc_price($taxLineValue),
        ];
