...
namespace Inpsyde\PayoneerForWoocommerce\Taxes\Modifier;
...
class LineItemTaxModifier extends AbstractTaxModifier
{
    public function modify(): void
    {
        ...
        foreach ($this->modifiers as $modifier) {
            $taxModifierLineItemConnector = TaxModifierLineItemConnector::fromCart(
                $this->cart,
                $modifier->getProductId()
            );
            ...
            if (!(isset($this->cart->cart_contents[$lineItemKey]) && is_array($this->cart->cart_contents[$lineItemKey]))) {
                ...
                do_action(
                    'payoneer-checkout.cart_item_not_found_for_tax_modifier',
                    $this->cart,
                    $lineItemKey
                );
                continue;
            ...
            [$lineItemKey]['line_subtotal_tax'] = $modifier->getTaxAmount();
    ...
    private function setLineItemTaxData(string $lineItemKey, string $property, float $newValue): void
    {
        ...
        foreach (array_keys($lineItemTaxData) as $lineItemSubTotalKey) {
            if (
                isset($this->cart->cart_contents[$lineItemKey]['line_tax_data']
                    [$property][$lineItemSubTotalKey]) &&
                is_array($this->cart->cart_contents[$lineItemKey]['line_tax_data']) &&
                is_array($this->cart->cart_contents[$lineItemKey]['line_tax_data'][$property])
            ) {
                $this->cart->cart_contents[$lineItemKey]['line_tax_data']
                [$property][$lineItemSubTotalKey] = $newValue;
            }
        }
    }
}
