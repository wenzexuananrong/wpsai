...
use Inpsyde\PayoneerForWoocommerce\Taxes\Modifier\AbstractTaxModifier;
use Inpsyde\PayoneerForWoocommerce\Taxes\Modifier\DiscountTaxModifier;
use Inpsyde\PayoneerForWoocommerce\Taxes\Modifier\OrderFeeTaxModifier;
use Inpsyde\PayoneerForWoocommerce\Taxes\Modifier\OrderLineItemTaxModifier;
use Inpsyde\PayoneerForWoocommerce\Taxes\Modifier\OrderShippingTaxModifier;
use Inpsyde\PayoneerForWoocommerce\Taxes\Modifier\TaxModifierItem;
...
class TaxesModule implements ServiceModule, ExecutableModule, ExtendingModule
{
    ...
    protected function registerTaxModifiers(ContainerInterface $container): void
    {
        add_action('woocommerce_after_calculate_totals', function () use ($container) {
            ...
            try {
                ...
                do_action('payoneer-checkout.invalid_tax_configuration', [
            ...
            do_action(
                'payoneer-checkout.cart_taxes_modified',
                $list,
                $cart,
                ...$modifiers
            );
    ...
    private function currentPaymentMethodIsPayoneer(): bool
    {
        ...
        $userOnFrontendIsPayoneer = !is_admin()
            && $session->get('chosen_payment_method')
            === 'payoneer-checkout';
    ...
    public function registerPayForOrderPayoneerLines(ContainerInterface $container): void
    {
        ...
        add_filter(
            ...
            function (array $totals, WC_Order $order) use ($container) {

                if (
                    ($this->isEmailTemplate &&
                        $order->get_payment_method() !== 'payoneer-checkout'
                    )
                    ||
                    (
                        !$container->get('wc.is_checkout_pay_page') &&
                        !$this->isEmailTemplate
                    )
                ) {
                    return $totals;
    ...
    public function orderPayoneerModifiers(\WC_Order $order, ContainerInterface $container): void
    {
        if (
            $order->get_payment_method() !== 'payoneer-checkout' ||
            $container->get('wc.settings.price_include_tax')
        ) {
            return;
        }

        /** @var string $listSessionFieldName */
        $listSessionFieldName = $container->get(
            'inpsyde_payment_gateway.list_session_field_name'
        );
        ...
        foreach ($products as $product) {
            ...
            if ($product['type'] === ProductType::OTHER) {
                if (!empty($product['name'])) {
                    ...
                    throw new MissingListDataException(
                        'List item shipping is missing parameter: name.'
                    );
                }
    ...
    {
        add_action(
            'payoneer-checkout.payment_aborted',
        ...
        add_filter(
            'wc_get_template',
            function (string $template, string $templateName, array $args) {
                /**
    ...
    protected function registerPayoneerTaxRateForMOR(ContainerInterface $container): void
    {
        add_filter('woocommerce_find_rates', function (array $rates) use ($container) {
            if (
                !$this->currentPaymentMethodIsPayoneer() ||
                $this->isPaymentAborted() ||
                $container->get('wc.settings.price_include_tax')
            ) {
                return $rates;
            }
            ...
            return [
                self::TAX_RATE_ID => [
                    'rate' => 0,
                    'label' => __('Tax', 'payoneer-checkout'),
                    'shipping' => 'yes',
                    'compound' => 'yes',
                ],
            ];
    ...
    protected function validateCartAmounts(\WC_Cart $cart, ListInterface $list): void
    {
        ...
        if ($payment->getAmount() !== (float)$cart->get_total('')) {
            throw new ListAndDisplayedAmountsMismatchException(
                sprintf(
                    'Cart amount %1$f doesn\'t match List payment amount %2$f.',
                    $cart->get_total(''),
                    $payment->getAmount()
                )
            );
        ...
        if ($payment->getTaxAmount() !== (float)$cart->get_taxes_total()) {
            throw new ListAndDisplayedAmountsMismatchException(
                sprintf(
                    'Cart tax amount %1$f doesn\'t match List payment amount %2$f.',
                    $cart->get_total_tax(),
