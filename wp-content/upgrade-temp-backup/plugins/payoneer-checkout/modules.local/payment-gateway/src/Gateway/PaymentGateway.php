...
use Inpsyde\PayoneerForWoocommerce\PaymentGateway\Gateway\RefundProcessor\RefundProcessorInterface;
use Inpsyde\PayoneerForWoocommerce\PaymentGateway\Gateway\Settings\SettingsFieldRendererInterface;
use Inpsyde\PayoneerForWoocommerce\PaymentGateway\Gateway\Settings\SettingsFieldSanitizerInterface;
...
class PaymentGateway extends WC_Payment_Gateway
{
    ...
    /**
     * A service able to validate API credentials.
     *
     * @var ValidatorInterface
     */
    protected $apiCredentialsValidator;
    ...
    public function __construct(
        ...
        ValidatorInterface $apiCredentialsValidator,
        MapInterface $options,
        string $optionKey,
        string $payoutIdFieldName,
        MerchantInterface $merchant,
        MerchantQueryInterface $merchantQuery,
        SaveMerchantCommandInterface $saveMerchantCommand,
        MerchantDeserializerInterface $merchantDeserializer
    ) {
    ...
    public function get_title(): string
    {
        ...
        if ($this->options->get('live_mode') === 'no') {
            $title = __('Test:', 'payoneer-checkout') . ' ' . $title;
        }
        return $title;
    }
    ...
    public function process_refund($orderId, $amount = \null, $reason = '')
    {
        ...
        if (empty($amount)) {
            return new WP_Error(
                'invalid_amount',
                __('Cannot refund an amount of 0,00', 'payoneer-checkout')
            );
        }
        ...
        try {
            $list = $this->refundProcessor->refundOrderPayment($order, $amount, $reason);
            $this->refundLongIdCache = $list->getIdentification()->getLongId();
        } catch (Exception $exception) {
            return new WP_Error(
                'failed_to_refund_order_payment',
                __('Failed to refund the order payment', 'payoneer-checkout')
            );
        }
    ...
    public function generate_settings_html($formFields = [], $echo = true)
    {
        ...
        foreach ($formFields as $key => $value) {
            ...
            try {
                 ...
                 */
                $fieldRenderer = $this->serviceContainer->get(
                    'inpsyde_payment_gateway.settings_field_renderer.' . $type
                );
    ...
    {
        $this->apiCredentialsValidator->validate([
            'code' => $code,
            'token' => $token,
            'url' => $url,
            'division' => $division,
    ...
    {
        $sets = [
            'sandbox' => $this->getFieldGroupValues('sandbox_credentials'),
            'live' => $this->getFieldGroupValues('live_credentials'),
        ];
    ...
    {
        add_action('woocommerce_after_order_refund_object_save', function (
            WC_Order_Refund $refund
        ) use ($payoutLongIdProvider): void {
    ...
    protected function isRefundOrderPaidWithPayoneer(WC_Order_Refund $refund): bool
    {
        ...
        return $parentOrder instanceof WC_Order &&
               $parentOrder->get_payment_method() === $this->id;
    }
    ...
    public function get_field_value($key, $field, $postData = [])
    {
        ...
        $postData = empty($postData) ? $_POST : $postData; // WPCS: CSRF ok, input var ok.
        ...
        try {
            ...
            try {
                 ...
                 */
                $sanitizer = $this->serviceContainer->get(
                    'inpsyde_payment_gateway.settings_field_sanitizer.' . $type
                );
