...
class BlogOptions implements MutableContainerInterface
{
    ...
    public function unset(string $key): void
    {
        ...
        $result = $blogId === null
            ? delete_option($key)
            : delete_blog_option($blogId, $key);

        if ($result === false) {
            throw new ContainerException(
                $this->__('Could not delete option "%1$s" for blog %2$s', [$key, $blogId === null ? 'null' : "#$blogId"]),
                0,
                null,
                $this
            );
    ...
    protected function getOption(string $name)
    {
        ...
        $value = $blogId === null
            ...
            : get_blog_option($blogId, $name, $default);
    ...
    protected function setOption(string $name, $value): void
    {
        ...
        /** @psalm-suppress PossiblyNullArgument */
        $isSuccessful = $blogId === null
            ? update_option($name, $value)
            : update_blog_option($blogId, $name, $value);
        if (!$isSuccessful) {
        ...
        if (!$isSuccessful) {
            throw new UnexpectedValueException(
                $this->__(
                    'New option value did not match the intended value in blog %3$s: "%1$s" VS "%2$s"',
                    [
                        print_r($value, true),
                        print_r($newValue, true),
                        $blogId === null ? 'null' : "#$blogId"
                    ]
                )
            );
        }
