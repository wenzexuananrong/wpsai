	...
	protected function render() {
		...
		if ( isset( $tabs_parent_settings['image']['id'] ) && $tabs_parent_settings['image']['id'] ) {
			...
			if ( woodmart_is_svg( $tabs_parent_settings['image']['url'] ) ) {
				$image_output = '<span class="svg-icon img-wrapper" style="width:' . esc_attr( $tabs_parent_settings['width'] ) . 'px; height:' . esc_attr( $custom_image_size['height'] ) . 'px;">' . woodmart_get_any_svg( $tabs_parent_settings['image']['url'], wp_rand( 999, 9999 ) ) . '</span>';
			}
		}
