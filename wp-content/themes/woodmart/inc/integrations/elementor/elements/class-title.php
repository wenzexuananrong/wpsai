...
class Title extends Widget_Base {
	...
	protected function render() {
		...
		if ( isset( $settings['image']['id'] ) && $settings['image']['id'] ) {
			$image_output = '<div class="img-wrapper">' . woodmart_otf_get_image_html( $settings['image']['id'], $settings['image_size'], $settings['image_custom_dimension'] ) . '</div>';

			if ( woodmart_is_svg( $settings['image']['url'] ) ) {
				$image_output = '<div class="img-wrapper"><span class="svg-icon" style="width:' . esc_attr( $custom_image_size['width'] ) . 'px; height:' . esc_attr( $custom_image_size['height'] ) . 'px;">' . woodmart_get_any_svg( $settings['image']['url'], rand( 999, 9999 ) ) . '</span></div>';
			}
		}
