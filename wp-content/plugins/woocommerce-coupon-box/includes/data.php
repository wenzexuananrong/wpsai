...
class VI_WOOCOMMERCE_COUPON_BOX_DATA {
	private $params;
	private $default;

	/**
	 * VI_WOOCOMMERCE_COUPON_BOX_DATA constructor.
	 * Init setting
	 */
	public function __construct() {
		...
		$this->default = array(
			...
			'wcb_email_template_customizer_enable' => 0,
			'wcb_email_template'                   => '',
			'wcb_email_enable'                     => 1,
...
//			'wcb_constantcontact_list'        => [],
			...
			'wcb_title'                         => 'WANT <span>10% OFF?</span>',
			'wcb_title_size'                    => '94',
			'wcb_title_space'                   => '20',
			'wcb_color_header'                  => '',
			'wcb_bg_header'                     => '',
			...
			'wcb_body_bg'                   => '#f4fbff',
			'wcb_body_bg_img'               => VI_WOOCOMMERCE_COUPON_BOX_IMAGES . 'bg-sale.jpg',
			'wcb_body_bg_img_repeat'        => 'no-repeat',
			'wcb_body_bg_img_size'          => 'cover',
			'wcb_body_bg_img_position'      => '0% 0%',
			'wcb_body_text_color'           => '#ffffff',
			...
			'wcb_button_border_radius'      => 4,
			'wcb_email_input_placeholder'   => 'Enter your email address',
			'wcb_email_input_border_radius' => 4,
			'wcb_email_button_space'        => 0,
			'wcb_email_input_color'         => '#000000',
			'wcb_email_input_bg_color'      => '',
			...
			'wcb_register_account_checkbox'         => 1,
			'wcb_register_account_checkbox_checked' => 1,
			'wcb_register_account_message'          => 'I agree register a new account',
			'wcb_restrict_domain'                   => '',
			'wcb_gdpr_checkbox'                     => 1,
			...
			'wcb_input_additional'           => 0,
			'wcb_input_additional_required'  => 0,
			'wcb_input_additional_type'      => 'text',
			'wcb_input_additional_label'     => 'Additional information',
			'wcb_custom_input_border_radius' => 0,
			'wcb_custom_input_color'         => '#000',
			'wcb_custom_input_bg_color'      => '#fff',
			...
			'wcb_recaptcha_position'            => 'after',
			'wcb_never_reminder_enable'         => 0,
			'wcb_no_thank_button_enable'        => 0,
			'wcb_no_thank_button_title'         => 'No, thanks',
			'wcb_no_thank_button_color'         => '#fff',
			'wcb_no_thank_button_bg_color'      => '#5e656b',
			'wcb_no_thank_button_border_radius' => '4',
			'email_template'                    => '',
			'email_template_for_register'       => '',
			'wcb_register_account'              => 0,
			'wcb_register_email_content'        => 'Your account has been created.

Here is your credential:

Username: {wcb_user_name}
...
Password: {wcb_password}'
	...
	public function get_language() {
		return $this->params['wcb_language'];
	}

	public function get_params( $name = "", $language = "" ) {
		$language = apply_filters( 'woo_coupon_box_params_language', $language, $name );
		...
		if ( ! $name ) {
			...
			if ( $language ) {
				...
				if ( isset( $this->params[ $name_language ] ) ) {
					return apply_filters( 'woo_coupon_box_params_' . $name_language, $this->params[ $name_language ] );
				} else {
					return apply_filters( 'woo_coupon_box_params_' . $name_language, $this->params[ $name ] );
				}
			} else {
				return apply_filters( 'woo_coupon_box_params_' . $name, $this->params[ $name ] );
			}
		} else {
			return false;
		}
	...
	public function get_default( $name = "" ) {
		if ( ! $name ) {
			...
			return apply_filters( 'woo_coupon_box_params_default_' . $name, $this->default[ $name ] );
		} else {
			return false;
		}
	}
	...
	public static function get_email_templates( $type = 'wcb_email' ) {
		return self::email_template_customizer_active() ? viwec_get_emails_list( $type ) : array();
	}
}

new VI_WOOCOMMERCE_COUPON_BOX_DATA();
