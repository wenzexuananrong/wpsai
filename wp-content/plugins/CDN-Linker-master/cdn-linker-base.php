...
class URI_changer
{
	 ...
	 */
	protected function rewrite_single($match,$dirs) {
		...
		return str_replace($blog_url .'/'. $dirs, $this->get_target_url->for_source($match[0]), $match[0]);
	...
	public function rewrite(&$content) {
		...
		$new_content = preg_replace_callback($regex, function ($matches) use ($dirs) {
			return $this->rewrite_single($matches, $dirs);
		}, $content);
...
function register_as_output_buffer_handler() {
	...
	ob_start(function($content) use ($rewriter) {
