jQuery(document).ready(function ($) {
    'use strict';
    /**
     * @var woo_orders_tracking_import_params
     */
        ...
        $('input[name="woo_orders_tracking_import"]').on('click', function (e) {
            ...
            if (empty_required_fields.length > 0) {
                if (empty_required_fields.length === 1) {
                    alert(woo_orders_tracking_import_params.i18n_required_field.replace('%s', empty_required_fields[0]))
                } else {
                    alert(woo_orders_tracking_import_params.i18n_required_field.replace('%s', empty_required_fields.join(', ')));
                }
                e.preventDefault();
                return false;
            }
            ...
            success: function (response) {
                ...
                switch (response.status) {
                    ...
                    case 'finish':
                        ...
                        alert(woo_orders_tracking_import_params.i18n_import_completed);
                        $('.woo-orders-tracking-view-log-after-import').fadeIn(300);
                        break;
                    case 'error':
                        $progress.progress('set error');
