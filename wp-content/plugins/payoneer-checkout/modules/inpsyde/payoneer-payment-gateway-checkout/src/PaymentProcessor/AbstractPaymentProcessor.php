...
abstract class AbstractPaymentProcessor implements PaymentProcessorInterface
{
    ...
    public function processPayment(WC_Order $order) : array
    {
        ...
        try {
            ...
            $exceptionWrapper = new Exception(__('API call failed. Please try again or contact the shop admin. Error details can be found in logs.', 'payoneer-checkout'), $exception->getCode(), $exception);
    ...
    protected function handleFailedPaymentProcessing(WC_Order $order, $error = null) : array
    {
        ...
        do_action('payoneer-checkout.payment_processing_failure', ['order' => $order, 'exception' => $error]);
    ...
    protected function produceErrorMessageFromException(\Throwable $exception, string $fallback) : string
    {
        if ($this->misconfigurationDetector->isCausedByMisconfiguration($exception)) {
            return __('Failed to initialize Payoneer session. Payoneer Checkout is not configured properly.', 'payoneer-checkout');
        }
        $previous = $exception;
        do {
