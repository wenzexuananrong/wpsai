...
class ListSessionInitializer
{
    public function __invoke(ContainerInterface $container)
    {
        ...
        add_action('woocommerce_checkout_update_order_review', function () {
             ...
             */
            $this->hookOnce('woocommerce_after_calculate_totals', static function () : void {
                $context = new CheckoutContext();
                do_action('payoneer-checkout.init_list_session', $context, \true);
            });
        }, 11);
        ...
        add_action('payoneer-checkout.init_list_session', static function (ContextInterface $context, bool $addNotice = \true) use($container) {
            try {
                ...
                $misconfigurationDetected = did_action('payoneer-checkout.payment_gateway_misconfiguration_detected');
                if ($addNotice && !$misconfigurationDetected) {
                    $message = __('The selected payment method for your order is not available at the moment. Please select another payment method.', 'payoneer-checkout');
