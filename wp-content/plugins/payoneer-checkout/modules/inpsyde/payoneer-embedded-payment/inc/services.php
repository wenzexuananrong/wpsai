...
use Syde\Vendor\Inpsyde\PayoneerForWoocommerce\EmbeddedPayment\Settings\CssField;
...
return static function () : array {
    return [
        ...
        'embedded_payment.widget.css_url' => new Factory(['embedded_payment.path.css', 'core.main_plugin_file'], static function (string $cssPath, string $pluginMainFile) : string {
            return \plugins_url($cssPath . 'widget.css', $pluginMainFile);
        }),
        'embedded_payment.widget.asset.template.options' => new Factory(['checkout.payment_gateway', 'embedded_payment.widget.asset.template.extra_options'], static function (\WC_Payment_Gateway $gateway, array $extraCssOptions) : array {
            $options = ['checkout_css_custom_css' => $gateway->get_option('checkout_css_custom_css', '')];
        ...
        'embedded_payment.widget.asset.template.extra_options' => new Value(['extra_css' => '']),
        'embedded_payment.widget.custom_css_url' => new Factory(['embedded_payment.widget.asset.processor', 'embedded_payment.widget.asset.template.location', 'embedded_payment.widget.asset.template.options'], static function (AssetProcessorInterface $assetProcessor, string $cssPath, array $cssOptions) : string {
            /** @psalm-suppress InvalidArgument */
            /** @psalm-suppress MixedArgumentTypeCoercion */
            return (string) $assetProcessor->process($cssPath, $cssOptions);
        }),
        'embedded_payment.widget_script_data' => new Factory(['core.list_url_container_id', 'checkout.list_url_container_attribute_name', 'core.payment_fields_container_id', 'embedded_payment.assets.widget.css_url', 'embedded_payment.widget.custom_css_url', 'embedded_payment.widget.button_id', 'checkout.payment_flow_override_flag', 'checkout.on_error_refresh_fragment_flag', 'wc.order_under_payment', 'embedded_payment.pay_order_error_flag'], static function (string $listUrlContainerId, string $listUrlContainerAttributeName, string $paymentFieldsContainerId, string $widgetCssUrl, string $customCssUrl, string $buttonId, string $hostedModeOverrideFlag, string $onErrorRefreshFragmentFlag, int $payForOrderId, string $payOrderErrorFlag) : array {
            return ['listUrlContainerId' => $listUrlContainerId, 'listUrlContainerAttributeName' => $listUrlContainerAttributeName, 'paymentFieldsContainerId' => $paymentFieldsContainerId, 'payButtonId' => $buttonId, 'widgetCssUrl' => $widgetCssUrl, 'developmentMode' => \false, 'isPayForOrder' => is_wc_endpoint_url('order-pay'), 'cssUrl' => $customCssUrl, 'hostedModeOverrideFlag' => $hostedModeOverrideFlag, 'onErrorRefreshFragmentFlag' => $onErrorRefreshFragmentFlag, 'payForOrderId' => $payForOrderId, 'payOrderErrorFlag' => $payOrderErrorFlag];
        }),
        'embedded_payment.pay_order_error_flag' => new Value('payoneer-checkout-on-before-server-error'),
        'embedded_payment.widget.asset.template.name' => new Value('custom.css'),
        'embedded_payment.widget.asset.template.location' => new StringService('checkout/static/templates/{template_name}', ['template_name' => 'embedded_payment.widget.asset.template.name']),
        ...
        'embedded_payment.assets.can_enqueue' => new FuncService(['wc.is_checkout', 'inpsyde_payment_gateway.is_enabled'], static function (bool $isCheckout, bool $isGatewayEnabled) : bool {
            return $isCheckout and $isGatewayEnabled;
        }),
        'embedded_payment.assets.css.websdk' => new Factory(['embedded_payment.assets.css.websdk.url', 'embedded_payment.assets.can_enqueue'], static function (string $webSdkCssUrl, callable $canEnqueue) : Style {
        ...
        'embedded_payment.assets.css.checkout' => new Factory(['core.main_plugin_file', 'embedded_payment.path.assets', 'embedded_payment.assets.can_enqueue'], static function (string $mainPluginFile, string $assetsPath, callable $canEnqueue) : Style {
            $url = \plugins_url($assetsPath . 'payoneer-checkout.css', $mainPluginFile);
        ...
        'embedded_payment.assets' => new Factory(['embedded_payment.assets.js.websdk', 'embedded_payment.assets.css.websdk', 'embedded_payment.assets.js.checkout', 'embedded_payment.assets.css.checkout'], static function (Script $webSdkJs, Style $webSdkCss, Script $checkoutJs, Style $checkoutCss) : array {
         ...
         *
         */
        'embedded_payment.payment_fields_renderer.placeholder' => new Constructor(WidgetPlaceholderFieldRenderer::class, ['inpsyde_payment_gateway.payment_fields_container_id']),
        'embedded_payment.payment_fields_renderer.list_url' => new Factory(['list_session.manager', 'inpsyde_payment_gateway.list_url_container_id', 'core.list_url_container_attribute_name'], static function (ListSessionProvider $listSessionProvider, string $containerId, string $attributeName) : PaymentFieldsRendererInterface {
        ...
        'inpsyde_payment_gateway.payment_processor' => new Factory(['inpsyde_payment_gateway.gateway', 'inpsyde_payment_gateway.update_command_factory', 'list_session.manager', 'inpsyde_payment_gateway.charge_id_field_name', 'inpsyde_payment_gateway.transaction_id_field_name', 'checkout.payment_flow_override_flag', 'embedded_payment.misconfiguration_detector', 'checkout.security_token_generator', 'checkout.order.security_header_field_name'], static function (PaymentGateway $paymentGateway, WcOrderBasedUpdateCommandFactoryInterface $updateCommandFactory, ListSessionManager $listSessionManager, string $chargeIdFieldName, string $transactionIdFieldName, string $hostedModeOverrideFlag, MisconfigurationDetectorInterface $misconfigurationDetector, TokenGeneratorInterface $tokenGenerator, string $tokenKey) : PaymentProcessorInterface {
            return new EmbeddedPaymentProcessor($paymentGateway, $updateCommandFactory, $listSessionManager, $listSessionManager, $tokenGenerator, $tokenKey, $chargeIdFieldName, $transactionIdFieldName, $hostedModeOverrideFlag, $misconfigurationDetector);
        }),
        ...
        'embedded_payment.ajax_order_pay.payment_action' => new Factory(['inpsyde_payment_gateway.gateway'], static function (\WC_Payment_Gateway $paymentGateway) : AjaxPayAction {
            return new AjaxPayAction($paymentGateway);
        }),
        /**
         * WC Settings API fields
         */
        'inpsyde_payment_gateway.settings_field_renderer.css' => new Constructor(CssField::class),
    ];
};
