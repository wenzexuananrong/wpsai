...
class Package
{
    ...
    public function connect(Package $package) : bool
    {
        ...
        if (array_key_exists($packageName, $this->connectedPackages)) {
            do_action($this->hookName(self::ACTION_FAILED_CONNECTION), $packageName, new \WP_Error('already_connected', 'already connected', $errorData));
            return \false;
        }
        // Don't connect, if already booted or boot failed
        if (in_array($this->status, [self::STATUS_BOOTED, self::STATUS_FAILED], \true)) {
            $this->connectedPackages[$packageName] = \false;
            do_action($this->hookName(self::ACTION_FAILED_CONNECTION), $packageName, new \WP_Error('no_connect_status', 'no connect status', $errorData));
            return \false;
        }
