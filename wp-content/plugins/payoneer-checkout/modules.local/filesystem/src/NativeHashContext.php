    ...
    public function init(): void
    {
        ...
        if (!$context) {
            throw new UnexpectedValueException(
                sprintf('Failed to initialize hashing context with algo "%1$s"', $algo)
            );
        }
    ...
    public function update(string $data): void
    {
        ...
        if ($context === null) {
            throw new RangeException(
                sprintf(
                    'Context with algo "%1$s" must be initialized before it can be updated',
                    $algo
                )
            );
        }
        ...
        if (!$result) {
            throw new UnexpectedValueException(
                sprintf(
                    'Failed to update context with algo "%1$s" with %2$d bits of data',
                    $algo,
                    strlen($data)
                )
            );
