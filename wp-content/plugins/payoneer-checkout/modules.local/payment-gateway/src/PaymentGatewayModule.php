...
class PaymentGatewayModule implements ServiceModule, ExecutableModule, ExtendingModule
{
    ...
    public function run(ContainerInterface $container): bool
    {
        ...
        foreach (
            [
            'inpsyde_payment_gateway.payment_processor',
            'inpsyde_payment_gateway.payment_fields_renderer',
            ] as $key
        ) {
            if (! $container->has($key)) {
                throw new \RuntimeException(
                    sprintf(
                        'Service "%1s" is expected to be overridden by other modules',
                        $key
                    )
                );
            }
        ...
        add_action('woocommerce_init', function () use ($container) {
            ...
            add_filter(
                ...
                static function (array $validStatuses, WC_Order $order) {
            ...
            if (is_admin()) {
                $delegate = new FuncService(
                    ...
                    \Closure::fromCallable([$this, 'addSandboxNotice'])
                );
        ...
        add_action('woocommerce_settings_saved', function () use ($container) {
            $delegate = new FuncService(
                ...
                \Closure::fromCallable([$this, 'reloadSettingsPage'])
            );
            /** @psalm-suppress MixedFunctionCall */
            $delegate($container)();
        });
        ...
        add_action('woocommerce_settings_start', function () use ($container) {
            $delegate = new FuncService(
                ...
                \Closure::fromCallable([$this, 'transferGatewayErrorsAfterReload'])
            );
            /** @psalm-suppress MixedFunctionCall */
            $delegate($container)();
        });
     ...
     * @return void
     */
    public function reloadSettingsPage(
        bool $isSettingsPage,
        PaymentGateway $paymentGateway,
        UriInterface $settingsUrl
    ): void {
     ...
     * @return void
     */
    public function transferGatewayErrorsAfterReload(
        bool $isSettingsPage,
        PaymentGateway $paymentGateway
    ): void {
    ...
    public function addSandboxNotice(bool $liveMode, UriInterface $settingsPageUrl): void
    {
        ...
        add_action('all_admin_notices', static function () use ($settingsPageUrl): void {
            ...
            $disableTestMode = sprintf(
                ...
                esc_html__(
                    '%1$sEnable live mode%2$s when you are ready to take live transactions.',
                    'payoneer-checkout'
                ),
                $aTagOpening,
                '</a>',
            ...
            printf(
                ...
                esc_attr($class),
                esc_html__('Payoneer Checkout Live mode is disabled', 'payoneer-checkout'),
