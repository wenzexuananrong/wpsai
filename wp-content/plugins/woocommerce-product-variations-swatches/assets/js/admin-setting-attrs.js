jQuery(document).ready(function ($) {
    ...
    $('.vi-wpvs-attribute-terms-list-data-table').map(function () {
        let $table = $(this),
            t_attribute_name = $table.data('attribute_name');
        data_attribute[t_attribute_name] = '[]';

        let current_data_table = $table.DataTable({
                aLengthMenu: [
                    [25, 50, 100, 200, 500],
                    [25, 50, 100, 200, 500]
                ],
        ...
        vi_load_terms_for_attribute(current_data_table, $table, t_attribute_name);

    });

    function vi_load_terms_for_attribute(current_data_table, table_list, attribute_name, page = 1) {
        $.ajax({
            url: viwpvs_admin_attrs_js.ajax_url,
            type: 'post',
            data: {
                action: 'vi_wvps_load_terms_for_attribute',
            ...
            success: function (response) {
                if (response.status === 'continue') {
                    ...
                    let new_content = JSON.parse(response.content),
                        old_content = JSON.parse(data_attribute[attribute_name]);
                    const mergeResult = old_content.concat(new_content);
                    data_attribute[attribute_name] = JSON.stringify(mergeResult);

                    vi_load_terms_for_attribute(current_data_table, table_list, attribute_name, page);
                } else if (response.status === 'successfully') {
                    let attributes_list = JSON.parse(data_attribute[attribute_name]);

                    for (let attribute_index of attributes_list) {
                        let term_id = attribute_index['term_id'],
                            term_name = attribute_index['term_name'],
                            terms_color_separator = attribute_index['terms_color_separator'],
                            terms_colors = attribute_index['terms_colors'],
                            terms_color = attribute_index['terms_color'],
                            terms_img_id = attribute_index['terms_img_id'],
                            terms_img_src = attribute_index['terms_img_src'],
                            title_class = attribute_index['title_class'];
                        let rows_color = ``;
                        if (terms_colors && Array.isArray(terms_colors) && terms_colors.length > 0) {
                            for (let terms_color_item of terms_colors) {
                                rows_color += `
                                <tr>
                                    <td>
                                        <input type="text"
                                               ...
                                               value="${terms_color_item}">
                                    </td>
                                    <td>
                                        <span class="vi-wpvs-term-color-action-clone vi-ui positive button">Clone</span>
                                        <span class="vi-wpvs-term-color-action-remove vi-ui negative button">Remove</span>
                                    </td>
                                </tr>`;
                            }
                        } else {
                        ...
                        let class_icon = terms_img_id ? '' : ' vi-wpvs-hidden';

                        current_data_table.row.add($(`
                        <tr class="vi-wpvs-accordion-wrap vi-wpvs-accordion-term-wrap vi-wpvs-accordion-term-wrap-${term_id}" data-term_id="${term_id}">
                            <td>${term_name}</td>
                            <td>
                                <div class="vi-ui styled fluid accordion ">
                                    ...
                                    <div class="${title_class}">
                                        ...
                                        <span class="vi-wpvs-attribute-value-image-config">Image <span class="vi-wpvs-attribute-value-image-preview"></span>
                                        </span>
                                    </div>
                                    <div class="content">
                                        <div class="vi-wpvs-attribute-value-content-wrap vi-wpvs-attribute-value-content-image-wrap">
                                            <input type="hidden" name="vi_attribute_images" class="vi_attribute_image" value="${terms_img_id}">
                                            <div class="vi-attribute-image-wrap vi-attribute-edit-image-wrap vi-wpvs-term-image-upload-img">
                                                <span class="vi-attribute-edit-image-preview vi-attribute-image-preview">
                                                    <img src="${terms_img_src}" data-src_placeholder="">
                                                </span>
                                                <i class="vi-attribute-image-remove times circle outline icon ${class_icon}"></i>
                                                <div class="vi-attribute-image-add-new">Upload/Add an image</div>
                                            </div>
                                            <p class="description">Choose an image</p>
                                        </div>
                                    </div>
                                    <div class="${title_class}">
                                        <i class="dropdown icon"></i>
                                        <span class="vi-wpvs-attribute-value-color-config">Color<span class="vi-wpvs-attribute-value-color-preview"></span>
                                        </span>
                                    </div>
                                    <div class="content">
                                        <div class="vi-wpvs-attribute-value-content-wrap vi-wpvs-attribute-value-content-color-wrap">
                                            <table class="form-table">
                                                <tbody>
                                                <tr>
                                                    <td>Color separator</td>
                                                    <td>
                                                        <select name="vi_attribute_color_separator"
                                                                id="vi_attribute_color_separator_${term_id}"
                                                                class="vi-ui fluid dropdown vi_attribute_color_separator">
                                                            <option value="1" >Basic horizontal</option>
                                                            <option value="2" >Basic vertical</option>
                                                            <option value="3" >Basic diagonal left</option>
                                                            <option value="4" >Basic diagonal right</option>
                                                            <option value="5" >Hard lines horizontal</option>
                                                            <option value="6" >Hard lines vertical</option>
                                                            <option value="7" >Hard lines diagonal left</option>
                                                            <option value="8" >Hard lines diagonal right</option>
                                                        </select>
                                                    </td>
                                                </tr>
                                                <tr>
                    ...
                    current_data_table.on('draw.dt', function () {
                        ...
                        duplicateItem();
                        removeItem();
                        $(current_data_table.rows({page: 'current'}).nodes()).map(function (i) {
                        })
                    });
                    $('.vi-ui.dropdown').dropdown();
                    table_list.closest('.vi-wpvs-accordion-term-wrap-wrap').find('.active.dimmer').remove();
                    table_list.removeClass('loading');
                    UploadImage();
                    handleColorPicker();
                    duplicateItem();
                    removeItem();
