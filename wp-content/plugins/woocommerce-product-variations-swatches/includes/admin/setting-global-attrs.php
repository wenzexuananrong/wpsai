...
class VIWPVS_WOOCOMMERCE_PRODUCT_VARIATIONS_SWATCHES_Admin_Setting_Global_Attrs {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_vi_wvps_load_terms_for_attribute', array( $this, 'load_terms_for_attribute' ) );
...
//			'offset'     => 0,
		...
		if ( $count ) {
			...
			$loader              = '';
			$table_class         = array( 'vi-wpvs-attribute-terms-list-table', 'loading' );
			if ( $count > 25 ) {
            ...
            <div class="<?php echo esc_attr( implode( ' ', $taxonomy_term_class ) ) ?>">
				<?php echo $loader; ?>
                <table class="vi-ui celled center aligned table <?php echo esc_attr( implode( ' ', $table_class ) ) ?>" data-attribute_name="<?php echo esc_attr( $attribute_name ); ?>">
                    <thead>
                    <tr>
	...
	private function terms_settings_html_row( $term, $placeholder_img_src, $vi_default_colors ) {
		...
		$title_class[] = 'aligned';
		?>
        <tr class="<?php echo esc_attr( $term_class ); ?>" data-term_id="<?php echo esc_attr( $term->term_id ); ?>">
	...
	public function load_terms_for_attribute() {
		$response = array(
			'status'  => 'failed',
			'message' => '',
		);
		...
		$offset          = $page * $number_per_page - $number_per_page;
			...
			if ( ! empty( $terms ) ) {
				...
				foreach ( $terms as $term ) {
					...
					$rows_content[] = array(
						...
						'term_name'             => apply_filters( 'woocommerce_product_attribute_term_name', $term->name, $term ),
						'terms_color_separator' => $terms_color_separator,
						'terms_colors'          => $terms_colors,
						'terms_color'           => $terms_color,
						'terms_img_id'          => $terms_img_id,
						'terms_img_src'         => $terms_img_src,
						'title_class'           => implode( ' ', $title_class ),
					);
				}
				$response = array(
					'status'  => 'continue',
					'content' => json_encode( $rows_content ),
					'page'    => $page
				);
			} else {
				$response = array(
