...
class VIWPVS_WOOCOMMERCE_PRODUCT_VARIATIONS_SWATCHES_Frontend_Frontend {
	...
	protected static $single_slider_enable, $single_slider_show;

	public function __construct() {
		$this->settings             = VIWPVS_WOOCOMMERCE_PRODUCT_VARIATIONS_SWATCHES_DATA::get_instance();
		self::$single_slider_enable = $this->settings->get_params( 'single_product_list_slider' );
		self::$single_slider_show   = $this->settings->get_params( 'single_product_list_slider_min' ) ?: 5;
		self::$language             = '';
		...
		add_action( 'wp_enqueue_scripts', array( $this, 'wvps_wp_enqueue_scripts' ), $priority );
...
//			unset($result['image']);
	...
	public static function get_custom_cats_index( $attribute_name, $count_custom_rule, $custom_attribute_id, $custom_attribute_name, $custom_attribute_category, $product_cats ) {
		...
		$index          = false;
		$attribute_name = _wp_specialchars( strtolower( trim( $attribute_name ) ), ENT_QUOTES );

		for ( $i = 0; $i < $count_custom_rule; $i ++ ) {
			$rule_attr_id       = $custom_attribute_id[ $i ];
			$rule_attr_name     = _wp_specialchars( strtolower( trim( $custom_attribute_name[ $i ] ?? '' ) ), ENT_QUOTES );
...
//		$attribute = $args['attribute'];
	...
	public static function get_attribute_option_html( $attribute, $product, $options, $vi_attribute_settings, $vi_args, $type, $profile, $use_taxonomy_type = '', $change_product_image = '' ) {
		...
		$single_slider_enable = self::$single_slider_enable ? $settings->get_params( 'single_product_list_slider' ) : '';
		$single_slider_show   = self::$single_slider_show ?? $settings->get_params( 'single_product_list_slider_min' );
		$single_slider_show   = (int) $single_slider_show ?: 5;
		$single_slider_type   = $settings->get_params( 'single_product_list_slider_type' ) ?: array();

		$single_slider_type = empty( $single_slider_type ) ? array(
			'color',
			'image',
			'variation_img'
		) : $single_slider_type;
		...
		if ( in_array( $type, $single_slider_type ) && $single_slider_enable && is_product() ) {
			$div_class[] = 'vi-wpvs-variation-wrap-slider ';
		}
	...
	public function wvps_wp_enqueue_scripts() {
		...
		if ( $this->settings->get_params( 'single_product_list_slider' ) ) {
			wp_enqueue_style( 'dashicons' );
			wp_enqueue_script( 'flexslider' );
			if ( ! wp_style_is( 'vi-wpvs-flexslider-single', 'registered' ) ) {
				wp_enqueue_style( 'vi-wpvs-flexslider-single', VIWPVS_WOOCOMMERCE_PRODUCT_VARIATIONS_SWATCHES_CSS . 'vi_flexslider.min.css', array(), VIWPVS_WOOCOMMERCE_PRODUCT_VARIATIONS_SWATCHES_VERSION );
			}
		}
