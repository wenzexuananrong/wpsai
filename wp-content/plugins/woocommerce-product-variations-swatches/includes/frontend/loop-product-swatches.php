...
class VIWPVS_WOOCOMMERCE_PRODUCT_VARIATIONS_SWATCHES_Frontend_Loop_Product_Swatches {
	...
	protected $max_attr_items, $link_more_enable, $expand_attributes;
	 ...
	 * @param $product WC_Product
	 */
	public function get_loop_attribute_option_html( $product ) {
		self::$is_loop = true;
		...
		if ( $attributes && $count_attrs = count( $attributes ) ) {
			...
			$expand_attributes                = $this->expand_attributes ?? $this->settings->get_params( 'product_list_expand_attributes_enable' );
                ...
                <form action=""
                    ...
                    <table class="variations" cellspacing="0" cellpadding="0">
						...
						if ( $slider_enable ) {
							...
							foreach ( $attributes as $attribute_name => $options ) {
								$options          = apply_filters( 'vi_wpws_sort_options_before_limit', $options, $product, $attribute_name );
								$count_attr_items = count( $options );
								/*If feature expand attributes turn on  show all options and hide them by js*/
...
//								}
	...
	public function init() {
		...
		$this->expand_attributes  = $this->settings->get_params( 'product_list_expand_attributes_enable' );
...
//		set_query_var( 'viwpvs_position', $this->position );
		if ( ! wp_style_is( 'vi-wpvs-frontend-loop-product-style', 'registered' ) ) {
			...
			$css .= '.vi_wpvs_loop_variation_form .vi_variation_container .vi-wpvs-variation-wrap-wrap:not(.vi-wpvs-variation-wrap-slider-active),';
			$css .= '.vi_wpvs_loop_variation_form .vi_variation_container .vi-wpvs-variation-wrap-wrap:not(.vi-wpvs-variation-wrap-slider-active) .vi-wpvs-variation-wrap-loop.vi-wpvs-variation-wrap{';
		...
		if ( ! wp_script_is( 'vi-wpvs-frontend-loop-product-script', 'registered' ) ) {
			...
			if ( $this->link_more_enable ) {
				if ( $this->expand_attributes ) {
					$args['expand_attributes']   = $this->expand_attributes;
					$args['max_attribute_items'] = $this->max_attr_items;
				}
			}
