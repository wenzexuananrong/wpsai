...
class Purchasing implements Model_Interface {
    ...
    /**
     * Unset the needs processing for gift card item in the order.
     * This is needed in order for WC to set the order as "completed" when the order only contains gift card items and maybe other virtual items.
     *
     * @since 1.3.8
     * @access public
     *
     * @param bool        $value   Needs processing value.
     * @param \WC_Product $product Product object.
     * @return bool Filtered needs processing value.
     */
    public function unset_needs_processing_gift_card_item( $value, $product ) {
        if ( 'advanced_gift_card' !== $product->get_type() ) {
            return $value;
        }
    ...
    public function run() {
        ...
        add_filter( 'woocommerce_order_item_needs_processing', array( $this, 'unset_needs_processing_gift_card_item' ), 10, 2 );
    }
}
