...
class License extends Base_Model implements Model_Interface, Initiable_Interface, Activatable_Interface {
    use \AGCFW\Models\SLMW\Traits\License_Check;
    use \AGCFW\Models\SLMW\Traits\Notices;
    ...
    */
    ...
    public function process_license_response( $response, $context = 'check' ) {
        ...
        if ( empty( $response ) ) {
            return array(
                'status'         => 'fail',
                'message'        => __( 'Failed to activate license. Failed to connect to license server. Please contact plugin support.', 'advanced-gift-cards-for-woocommerce' ),
    ...
    public function activate_license( $activation_email, $license_key ) {
        $activation_url = add_query_arg(
            ...
            apply_filters( 'agcfw_license_activation_url', $this->_constants->LICENSE_ACTIVATION_ENDPOINT )
        );
        ...
        if ( empty( $result ) || ! property_exists( $result, 'status' ) ) {
            return new \WP_Error( 'agcfw_license_activation_failed', __( 'Failed to activate license. Failed to connect to activation access point. Please contact plugin support.', 'advanced-gift-cards-for-woocommerce' ) );
        }
        ...
        do_action( 'agcfw_after_activate_license', $response, $activation_email, $license_key );
    ...
    public function get_license_data( $all = false ) {
        ...
        return isset( $license_data['AGC'] ) ? $license_data['AGC'] : array();
    }
    ...
    public function get_all_license_status_for_display() {
        ...
        $labels       = array(
            'no_license' => __( 'No license found', 'advanced-gift-cards-for-woocommerce' ),
            'expired'    => __( 'Expired', 'advanced-gift-cards-for-woocommerce' ),
            'active'     => __( 'Active', 'advanced-gift-cards-for-woocommerce' ),
            'disabled'   => __( 'Disabled', 'advanced-gift-cards-for-woocommerce' ),
        );
    ...
    public function prepend_license_plugin_action_links( $links ) {
        ...
        if ( ( ! is_array( $license_data ) || empty( $license_data ) ) && $license_form_link ) {
            ...
            $renew_link        = sprintf( '<a href="%s">%s</a>', $renew_license_url, __( 'Renew License', 'advanced-gift-cards-for-woocommerce' ) );
    ...
    public function enqueue_license_scripts( $screen, $post_type ) {
        ...
        $this->_is_acfw_screen = ( ACFWF()->Notices->is_acfw_screen( $screen, $post_type ) || 'coupons_page_acfw-advanced-gift-cards' === $screen->id ) && 'edit-shop_coupon' !== $screen->id; // The coupons list screen is not treated as an AC screen.
        ...
        if ( ! empty( $active_notices ) ) {
            ...
            wp_localize_script(
                ...
                'agcLicense',
                array(
                    'dismiss_notice_nonce'  => wp_create_nonce( 'agcfw_dismiss_license_notice' ),
                    'refresh_license_nonce' => wp_create_nonce( 'agcfw_refresh_license_status' ),
                )
            );
        }
        ...
        if ( in_array( 'no_license_reminder', $active_notices, true )
            ...
            $reminder_vite = new Vite_App(
                'agc-license-reminder',
                'packages/agc-license-reminder/index.ts',
                array( 'wp-pointer' )
            );
            ...
            wp_localize_script(
                ...
                'agcLicenseReminder',
                array(
                    'nonce'          => wp_create_nonce( 'agcfw_dismiss_license_notice' ),
                    'licensePageUrl' => $this->_constants->get_license_page_url(),
                    'i18n'           => array(
                        'content'           => $this->_get_no_license_reminder_content(),
                        'close'             => __( 'Close', 'advanced-gift-cards-for-woocommerce' ),
                        'enter_license_key' => __( 'Enter License Key', 'advanced-gift-cards-for-woocommerce' ),
                    ),
                )
            );
        }
         ...
         */
        $tab = $_GET['tab'] ?? ''; // phpcs:ignore WordPress.Security
        if ( 'toplevel_page_advanced-coupons-network' === $screen->base && in_array( $tab, array( 'agc_license' ), true ) ) {
            $slmw_vite = new Vite_App(
                'agcfw_mu_license',
                'packages/agc-mu-license/index.ts',
                array( 'vex' ),
                array( 'vex', 'vex-theme-plain' )
            );
            $slmw_vite->enqueue();
            wp_add_inline_script( 'agcfw_mu_license', 'vex.defaultOptions.className = "vex-theme-plain"', 'after' );
            wp_localize_script(
                'agcfw_mu_license',
                'slmw_args',
                array(
    ...
    public function ajax_activate_license() {
        $post_data = $this->_helper_functions->validate_ajax_request(
            array(
                ...
                'nonce_action'        => 'agcfw_activate_license',
    ...
    */
    ...
    private function _get_active_premium_plugins() {
        ...
        foreach ( $plugins as $key => $plugin_name ) {
            ...
            if ( 'ACFW' === $key ) {
                $is_active = $this->_helper_functions->is_plugin_active( \ACFWF\Helpers\Plugin_Constants::PREMIUM_PLUGIN );
            } elseif ( 'LPFW' === $key ) {
    ...
    */
    ...
    public function initialize() {
        ...
        add_action( 'wp_ajax_agcfw_dismiss_license_notice', array( $this, 'ajax_dismiss_license_notice' ) );
        add_action( 'wp_ajax_agcfw_refresh_license_status', array( $this, 'ajax_refresh_license_status' ) );
    }
    ...
    public function run() {
        ...
        add_action( 'agcfw_license_check', array( $this, 'check_license' ) );
        add_action( 'agcfw_after_load_backend_scripts', array( $this, 'enqueue_license_scripts' ), 10, 2 );
