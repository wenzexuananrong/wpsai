...
trait License_Check {
    ...
    public function check_license( $force = false ) {
        $last_check = get_site_transient( 'agcfw_check_license_timeout', 0 );
        ...
        if ( $last_check && ( ( time() - $last_check ) < $time_limit ) ) {
            return new \WP_Error(
                'agcfw_license_check_too_soon',
                __( 'Please wait for a minute before checking the license status again.', 'advanced-gift-cards-for-woocommerce' )
            );
        }
        ...
        $check_url = add_query_arg(
            ...
            apply_filters( 'agcfw_license_check_url', $this->_constants->LICENSE_CHECK_ENDPOINT )
        );

        $args = apply_filters(
            'agcfw_license_check_option',
            array(
                'timeout' => 10, // seconds.
        ...
        set_site_transient( 'agcfw_check_license_timeout', time(), DAY_IN_SECONDS );
        ...
        do_action( 'agcfw_after_check_license', $license_data, $activation_email, $license_key );
    ...
    private function _schedule_license_check() {
        ...
        if ( \WC()->queue()->get_next( 'agcfw_license_check', array(), 'agcfw_license_check' ) instanceof \WC_DateTime ) {
            return;
        }
    ...
    public function ajax_refresh_license_status() {
        $post_data = $this->_helper_functions->validate_ajax_request(
            array(
                ...
                'nonce_action'    => 'agcfw_refresh_license_status',
        ...
        wp_send_json(
            $license_data['LPFW'] ?? array(
                'status'  => 'fail',
                'message' => __( 'Missing plugin license data.', 'advanced-gift-cards-for-woocommerce' ),
            )
        );
    }
}
