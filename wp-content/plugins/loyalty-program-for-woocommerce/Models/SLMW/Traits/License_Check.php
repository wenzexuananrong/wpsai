...
trait License_Check {
    ...
    public function check_license( $force = false ) {
        $last_check = get_site_transient( 'lpfw_check_license_timeout', 0 );
        ...
        if ( $last_check && ( ( time() - $last_check ) < $time_limit ) ) {
            return new \WP_Error(
                'lpfw_license_check_too_soon',
                __( 'Please wait for a minute before checking the license status again.', 'loyalty-program-for-woocommerce' )
            );
        }
        ...
        $check_url = add_query_arg(
            ...
            apply_filters( 'lpfw_license_check_url', $this->_constants->LICENSE_CHECK_ENDPOINT )
        );

        $args = apply_filters(
            'lpfw_license_check_option',
            array(
                'timeout' => 10, // seconds.
        ...
        set_site_transient( 'lpfw_check_license_timeout', time(), DAY_IN_SECONDS );
        ...
        do_action( 'lpfw_after_check_license', $license_data, $activation_email, $license_key );
    ...
    private function _schedule_license_check() {
        ...
        if ( \WC()->queue()->get_next( 'lpfw_license_check', array(), 'lpfw_license_check' ) instanceof \WC_DateTime ) {
            return;
        }
    ...
    public function ajax_refresh_license_status() {
        $post_data = $this->_helper_functions->validate_ajax_request(
            array(
                ...
                'nonce_action'    => 'lpfw_refresh_license_status',
        ...
        wp_send_json(
            $license_data['LPFW'] ?? array(
                'status'  => 'fail',
                'message' => __( 'Missing plugin license data.', 'loyalty-program-for-woocommerce' ),
            )
        );
    }
}
