...
class Earn_Points implements Model_Interface {
    ...
    public function earn_points_buy_product_action( $order_id ) {
        ...
        $entry_type = $this->_is_order_points_waiting_period() ? 'pending_earn' : 'earn';

        if ( ! $this->should_customer_earn_points( $user_id, $order ) || $this->_helper_functions->point_entry_exists( 'buy_product', $entry_type, $order->get_id() ) ) {
            return;
        }
    ...
    public function earn_points_high_spend_breakpoint( $order_id ) {
        ...
        if ( ! $this->should_customer_earn_points( $user_id, $order ) || $this->_helper_functions->point_entry_exists( 'high_spend', $entry_type, $order->get_id() ) ) {
            return;
        }
        ...
        $order->update_meta_data( $this->_constants->BREAKPOINTS_ENTRY_ID_META, $entry_id );
        $order->save_meta_data();
    }
    ...
    public function earn_points_order_within_period_action( $order_id ) {
        ...
        if ( ! $user_id || ! $this->should_customer_earn_points( $user_id, $order ) || $this->_helper_functions->point_entry_exists( 'within_period', $entry_type, $order->get_id() ) ) {
            return;
        }
