...
class VI_SCT_SALES_COUNTDOWN_TIMER_Admin_Checkout_Report {
	...
	public function __construct() {
		if ( is_plugin_active( 'woocommerce/woocommerce.php' ) ) {
			...
			$this->start_date = gmdate( 'Y-m-d', ( $current_time - 30 * 86400 ) );
			$this->end_date   = gmdate( 'Y-m-d', $current_time );
	...
	public function settings_callback() {
		$start_date = isset( $_REQUEST['start_date'] ) ? urldecode( wc_clean( wp_unslash( $_REQUEST['start_date'] ) ) ) : $this->start_date;
		$end_date   = isset( $_REQUEST['end_date'] ) ? urldecode( wc_clean( wp_unslash( $_REQUEST['end_date'] ) ) ) : $this->end_date;
        ...
        <div class="wrap">
            ...
            <div class="vi-ui secondary pointing menu">
                <a class="item <?php echo ! $active ? 'active' : ''; ?>"
                   href="<?php echo esc_url( admin_url( 'admin.php?page=sales-countdown-timer-checkout-report' ) ) ?>"><?php esc_html_e( 'General', 'sales-countdown-timer' ) ?></a>
                <a class="item <?php echo $active ? 'active' : ''; ?>"
                   href="<?php echo esc_url( admin_url( 'admin.php?page=sales-countdown-timer-checkout-report&subpage=orders' ) ) ?>"><?php esc_html_e( 'Orders', 'sales-countdown-timer' ) ?></a>
            </div>
	...
	public function admin_enqueue_scripts() {
		...
		if ( $page === 'sales-countdown-timer-checkout-report' ) {
			...
			$choose_time = isset( $_REQUEST['display_time'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['display_time'] ) ) : '';
			$choose_time = $choose_time ? $choose_time : 'minute';
			...
			$admin::enqueue_script(
				array(
					'semantic-ui-dropdown',
					'transition',
					'vi-sct-checkout-report-chart',
					'vi-sct-checkout-report-js'
				),
				...
				if ( $data ) {
					...
					if ( count( $data->label ) ) {
						...
						foreach ( $data->label as $label ) {
							$labels[]          = gmdate( "M d", $label );
							$query_date        = gmdate( 'Y-m-d', $label );
	...
	private function get_data_by_countdown_time1( $choose_time ) {
		...
		if ( isset( $_POST['_vi_sct_checkout_report'] ) ) {
			if ( wp_verify_nonce( wc_clean( wp_unslash( $_POST['_vi_sct_checkout_report'] ) ), 'vi_sct_checkout_report_nonce' ) ) {
				$start_date = isset( $_POST['start_date'] ) ? urldecode( wc_clean( wp_unslash( $_POST['start_date'] ) ) ) : $this->start_date;
				$end_date   = isset( $_POST['end_date'] ) ? urldecode( wc_clean( wp_unslash( $_POST['end_date'] ) ) ) : $this->end_date;
		...
		if ( $the_query->have_posts() ) {
			while ( $the_query->have_posts() ) {
				$the_query->the_post();
				$order_id        = get_the_ID();
				$countdown_order = wc_get_order( $order_id );
				$countdown_info  = $countdown_order->get_meta( 'sctv_checkout_countdown_info', true );
				$countdown_timer = $this->time_revert( $countdown_info['time'], $choose_time ) ? $this->time_revert( $countdown_info['time'], $choose_time ) : 0;
