...
class VI_SCT_SALES_COUNTDOWN_TIMER_Cron {
	public function __construct() {
		...
		add_action( 'wp_ajax_nopriv_visctr_doing_cron', array( $this, 'visctr_doing_cron' ) );
		add_action( 'wp_ajax_visctr_doing_cron', array( $this, 'visctr_doing_cron' ) );
	}

	public function visctr_doing_cron() {
	...
	private function sctv_schedule_time_shortcode( $id ) {
		...
		if ( $index >= 0 ) {
			...
			set_transient( 'sales_countdown_timer_params_' . $id, array(
				'sale_countdown_fom' => $countdown_time_start_t,
				'sale_countdown_to'  => $countdown_time_end_t
			), $countdown_time_end_t );
			$form_date                       = gmdate( 'Y-m-d', $countdown_time_start_t );
			$form_time                       = gmdate( 'H:i', $countdown_time_start_t );
			$to_date                         = gmdate( 'Y-m-d', $countdown_time_end_t );
			$to_time                         = gmdate( 'H:i', $countdown_time_end_t );
	...
	private function sctv_schedule_remove_sale_product( $id ) {
		...
		$loop_enable   = $product->get_meta( '_woo_ctr_enable_loop_countdown', true );
		if ( $loop_enable ) {
			$now               = current_time( 'timestamp' );
			$gmt_offset        = get_option( 'gmt_offset' );
			$time_offset       = $now - 3600 * $gmt_offset;
			$date_on_sale_from = $product->get_meta( '_sale_price_dates_from', true );
			$date_on_sale_to   = $product->get_meta( '_sale_price_dates_to', true );
			$countdown_time    = $date_on_sale_to - $date_on_sale_from;
			$loop_time_val     = (int) $product->get_meta( '_woo_ctr_loop_countdown_val', true );
			if ( $loop_time_val ) {
				$loop_time_type = $product->get_meta( '_woo_ctr_loop_countdown_type', true );
				$loop_time_val  = $this->get_loop_time_val( $loop_time_val, $loop_time_type );
			}
			...
			set_transient( 'sales_countdown_timer_params_product_' . $id, array(
				'sale_countdown_fom' => $date_on_sale_from_t1,
				'sale_countdown_to'  => $date_on_sale_to_t1
			), $date_on_sale_to_t );
			$form_time = gmdate( 'H:i', $date_on_sale_from_t );
			$to_time   = gmdate( 'H:i', $date_on_sale_to_t );
